...
export default function LeadGenerationPage(){
  ...
  const[importSrc,setImportSrc]=useState('indiamart');
 ...
 const saveImConfig = async () => {
  ...
  const handleImport=async(e:any)=>{var f=e.target.files?.[0];if(!f)return;e.target.value='';var t=await f.text();var lines=t.split('\n').filter(function(l:any){return l.trim()});if(lines.length<2){toast_('Invalid CSV','error');return}var s=0;var fl=0;for(var i=1;i<lines.length;i++){var cols=lines[i].split(',').map(function(c:any){return c.trim().replace(/^"|"$/g,'')});var d:any={name:cols[1]||'',phone:cols[4]||'',email:cols[5]||'',product:cols[2]||'',city:cols[3]||'',supplier:cols[6]||''};if(!d.phone){fl++;continue}try{var token=safeGetItem('token');var r=await fetch(API+'/leads/manual',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({businessId:bizId,source:importSrc,leadData:d})});var res=await r.json();if(res.success)s++;else fl++}catch{fl++}}toast_('Imported '+s+' leads, '+fl+' failed','success');fetchLeads()};
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
           <select value={importSrc} onChange={e=>setImportSrc(e.target.value)} className="text-xs border border-gray-300 rounded-lg px-2 py-2 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200">
